...
class _PointnetSAModuleFSBase(nn.Module):
    ...
    def forward(self,
        ...
        if new_xyz is None:
            ...
            # print(sample_idx_list[0][0])
            # print(sample_idx_list[0][0].shape)
            ...
            # print(xyz[0][sample_idx_list[0][0][0]])
        ...
        if self.confidence_mlp is not None:
            ...
            return new_xyz, new_features, new_scores, sample_idx
        

        return new_xyz, new_features, None, sample_idx
...
class FusionModule(nn.Module):
    def __init__(self, *, fusion_high_mlp: List[int], fusion_low_mlp: List[int], fusion_comp_mlp: List[int], bn: bool = True):
        ...
        for k in range(len(fusion_high_mlp)):
            if k == len(fusion_high_mlp) -1:
                high_mlp.extend([
                    nn.Linear(in_features=output_features, out_features=fusion_high_mlp[k], bias=True),
                    nn.Sigmoid(),
                ])
            
            elif k == 0:
                high_mlp.extend([
                    nn.Linear(in_features=320, out_features=fusion_high_mlp[k], bias=True),
                    nn.BatchNorm1d(512),
                    nn.ReLU(),
                ])
            ...
            output_features = fusion_high_mlp[k]
        self.fusion_high_mlp = nn.Sequential(*high_mlp)


        low_mlp = []
        for k in range(len(fusion_low_mlp)):
            if k == len(fusion_low_mlp) -1:
                low_mlp.extend([
                    nn.Linear(in_features=output_features, out_features=fusion_low_mlp[k], bias=True),
                    nn.Sigmoid(),
                ])
            
            elif k == 0:
                low_mlp.extend([
                    nn.Linear(in_features=320, out_features=fusion_low_mlp[k], bias=True),
                    nn.BatchNorm1d(512),
                    nn.ReLU(),
                ])
    ...
    def forward(self, high_feature: torch.Tensor, low_feature: torch.Tensor) -> torch.Tensor:
        ...
        concated = torch.cat((high_feature, low_feature), 2)
